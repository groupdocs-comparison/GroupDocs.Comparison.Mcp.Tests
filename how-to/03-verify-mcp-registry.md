...
The `GroupDocs.Comparison.Mcp` package is an **MCP server** (the csproj declares
...
[nuget.org/packages/GroupDocs.Comparison.Mcp](https://www.nuget.org/packages/GroupDocs.Comparison.Mcp)
...
```bash
curl -L -o pkg.nupkg \
  https://www.nuget.org/api/v2/package/GroupDocs.Comparison.Mcp/26.5.0
...
```json
{
  ...
  "name": "io.github.groupdocs-comparison/groupdocs-comparison-mcp",
  "version": "26.5.0",
  "packages": [ { "registryType": "nuget", "identifier": "GroupDocs.Comparison.Mcp", ... } ],
  "repository": { "url": "https://github.com/groupdocs-comparison/GroupDocs.Comparison.Mcp" }
}
```
...
3. Search for `groupdocs-comparison` or `comparison`.
4. The package should appear with the description from `server.json`:
   *"MCP server for GroupDocs.Comparison — compare PDF, Word, Excel, PowerPoint, and text documents and produce diff reports via AI agents."*
...
- `dnx` successfully resolves `GroupDocs.Comparison.Mcp@26.5.0` from nuget.org.
...
curl -s https://registry.modelcontextprotocol.io/v0/servers \
  | jq '.[] | select(.name | contains("groupdocs-comparison"))'
```
...
- `ToolDiscoveryTests.ServerInfo_AdvertisesGroupDocsComparisonMcp` — the server
  reports `serverInfo.name == "GroupDocs.Comparison.Mcp"` and a non-empty version
  that matches the package under test.
- `ToolDiscoveryTests.ListTools_ExposesReadAndCompare` — both MCP tools
  are advertised.
