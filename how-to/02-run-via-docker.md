...
- `ghcr.io/groupdocs-comparison/comparison-net-mcp` — GitHub Container Registry (primary)
- `docker.io/groupdocs/comparison-net-mcp` — Docker Hub (mirror)
...
```bash
docker pull ghcr.io/groupdocs-comparison/comparison-net-mcp:26.5.0   # pinned to exact version
docker pull ghcr.io/groupdocs-comparison/comparison-net-mcp:latest   # floats to most recent push
```
...
docker pull ghcr.io/groupdocs-comparison/comparison-net-mcp:latest
...
```bash
docker inspect ghcr.io/groupdocs-comparison/comparison-net-mcp:latest \
...
(
  ...
  sleep 2
    ...
    -v $(pwd)/documents:/data \
    ghcr.io/groupdocs-comparison/comparison-net-mcp:26.5.0 2>/dev/null
```

Expected: two JSON-RPC responses on stdout. The second includes `Compare`
and `GetDocumentInfo` with their descriptions and input schemas.
...
services:
  groupdocs-comparison-mcp:
    image: ghcr.io/groupdocs-comparison/comparison-net-mcp:26.5.0
    stdin_open: true
    tty: false
    environment:
...
Without the license, `Compare` still works — output is just watermarked
(see [01 — NuGet](01-install-from-nuget.md#license)). `GetDocumentInfo` is
...
docker inspect ghcr.io/groupdocs-comparison/comparison-net-mcp:26.5.0 \
  --format '{{json .Config}}' | jq

# Expected:
# - Entrypoint: ["dotnet", "GroupDocs.Comparison.Mcp.dll"]
...
| `manifest unknown` / can't pull image | Version tag doesn't exist on that registry | Check [ghcr.io/groupdocs-comparison/comparison-net-mcp](https://github.com/groupdocs-comparison/GroupDocs.Comparison.Mcp/pkgs/container/comparison-net-mcp) for available tags. |
| `:latest` ran but didn't pick up a new release | Docker reuses cached image — `docker run :latest` does not auto-pull | `docker pull ghcr.io/groupdocs-comparison/comparison-net-mcp:latest` before running, or add `--pull always` to your `docker run`. |
