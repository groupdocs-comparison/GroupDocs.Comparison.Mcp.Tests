...
#
# Launches `ghcr.io/groupdocs-comparison/comparison-net-mcp` as an interactive MCP
...
#                         stdin and verify both Compare and GetDocumentInfo
#                         tools are advertised. Returns 0 on success.
#
# Prerequisites:
...
#                           advertise `Compare` and `GetDocumentInfo`.
#   --license PATH          Mount a GroupDocs license file (enables
#                           compare; without it the server runs in
#                           evaluation mode)
...
IMAGE_REPO="ghcr.io/groupdocs-comparison/comparison-net-mcp"
...
# nothing to /data when only running compare / non-licensed compare.
...
if [ -n "$LICENSE_PATH" ]; then
    ...
    log_info "License mounted — compare will work in licensed mode"
else
    log_info "No license — compare will return evaluation-mode error (compare works fine)"
fi
...
if [ "$RUN_MODE" = "smoke" ]; then
    ...
    if echo "$response" | grep -q '"compare"' && echo "$response" | grep -q '"compare"'; then
        log_success "Compare and GetDocumentInfo tools advertised — smoke test PASSED"
        exit 0
    else
