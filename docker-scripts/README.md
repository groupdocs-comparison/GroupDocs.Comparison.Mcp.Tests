...
Complete bash script suite for running GroupDocs.Comparison.Mcp integration tests in both local and Docker environments. **Works on Windows (Git Bash/WSL), Linux, and macOS.**

## 📋 Overview

This folder contains production-ready bash scripts for testing the GroupDocs.Comparison.Mcp NuGet package across all scenarios:

- **ToolDiscovery** — MCP server handshake and tool listing
- **Compare** — Source vs target document diff (PDF/Word/Excel/PowerPoint), produces a marked-up result file plus a change-count summary
- **GetDocumentInfo** — File type, page count, file size, and per-page dimensions for a single source document (no comparison)
...
| `04_run-server-with-samples.sh` | **Interactive Docker MCP server** — Launches the published `comparison-net-mcp` container with `sample-docs/` mounted, for ad-hoc smoke testing. |
...
./02_test-all-scenarios.sh --filter Compare
...
**Output:**
```
Passed  ToolDiscoveryTests.ServerInfo_AdvertisesGroupDocsComparisonMcp
Passed  CompareTests.Compare_AuthoredPdf_ReturnsFileFormatAndProperties
Passed  ErrorHandlingTests.Compare_UnknownFile_ReturnsErrorListingAvailableFiles
...
...
services:
  comparison-server:
    image: ghcr.io/groupdocs-comparison/comparison-net-mcp:latest
    volumes:
...
```bash
# Unlock licensed-mode Compare tests
...
Without a license:
- `Compare_InEvaluationMode_ReturnsErrorResponse` — ✅ Passes (asserts graceful error)
- `Compare_Jpeg_WritesCleanOutput_Licensed` — ⏭️ Skipped
- `Compare_FollowUpReadOfCleanedFile_Succeeds_Licensed` — ⏭️ Skipped
...
| `Compare` | 3 tests | `./02_test-all-scenarios.sh --filter Compare` |
| `Compare` | 3 tests | `./02_test-all-scenarios.sh --filter Compare` |
...
docker build -t comparison-net-mcp:local ../../../GroupDocs.Comparison.Mcp/
...
# Docker utilities
docker_pull_image "ghcr.io/groupdocs-comparison/comparison-net-mcp:latest"
check_docker_daemon
...
```
Passed  ToolDiscoveryTests.ServerInfo_AdvertisesGroupDocsComparisonMcp
Passed  ToolDiscoveryTests.ListTools_ExposesReadAndCompare
Passed  ToolDiscoveryTests.AllTools_HaveNonEmptyDescriptionAndInputSchema
Passed  CompareTests.Compare_AuthoredPdf_ReturnsFileFormatAndProperties
Passed  CompareTests.Compare_Jpeg_ReturnsJpegFormat
Passed  CompareTests.Compare_AuthoredPdf_SurfacesKnownAuthorValue
Passed  CompareTests.Compare_InEvaluationMode_ReturnsErrorResponse
Passed  CompareTests.Compare_Jpeg_WritesCleanOutput_Licensed
Passed  CompareTests.Compare_FollowUpReadOfCleanedFile_Succeeds_Licensed
Passed  ErrorHandlingTests.Compare_UnknownFile_ReturnsErrorListingAvailableFiles
Passed  ErrorHandlingTests.Compare_CorruptedFile_DoesNotCrashServer
...
- [GroupDocs.Comparison.Mcp NuGet](https://www.nuget.org/packages/GroupDocs.Comparison.Mcp)
- [Integration Tests Guide](../how-to/06-run-integration-tests.md)
- [Docker Setup Guide](../how-to/02-run-via-docker.md)
- [Test Project](../src/GroupDocs.Comparison.Mcp.Tests/GroupDocs.Comparison.Mcp.Tests.csproj)
